...
@tree.command(
    name="draw",
    description="Generate an image with the Dalle3, Stable Diffusion, or Replicate model",
)
@app_commands.describe(
    prompt="The prompt for image generation", enhance="Enhance the prompt using AI (optional)"
)
@app_commands.choices(
    enhance=[
        app_commands.Choice(name="Yes", value="yes"),
...
async def play_command(interaction: discord.Interaction, search: str):
    await music.play(interaction, search)
