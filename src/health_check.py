...
async def health_check():
    ...
    required_vars = ["DISCORD_BOT_TOKEN", "ANTHROPIC_API_KEY"]
    ...
    for var, feature in optional_vars.items():
        if not os.getenv(var):
            logger.warning(f"Missing optional environment variable: {var} (disables {feature})")
    ...
    try:
        ...
        await loop.run_in_executor(
            ...
            lambda: anthropic_client.messages.create(
                model=CLAUDE_MODEL, max_tokens=10, messages=[{"role": "user", "content": "test"}]
            ),
        )
