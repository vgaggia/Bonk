...
async def generate_image_dalle(prompt, aspect_ratio=None):
    try:
        ...
        size = (
            ...
            else _DALLE_ASPECT_RATIO_TO_SIZE.get(aspect_ratio, "1024x1024")
        )
        ...
        image_data = await loop.run_in_executor(None, lambda: requests.get(image_url).content)
...
async def generate_image_sd(prompt, aspect_ratio):
    try:
        logger.debug(
            f"Generating image with Stable Diffusion 3. Prompt: {prompt}, Aspect Ratio: {aspect_ratio}"
        )
...
async def generate_image_replicate(prompt, aspect_ratio, model_id="black-forest-labs/flux-schnell"):
    try:
        logger.debug(
            f"Generating image with Replicate ({model_id}). Prompt: {prompt}, Aspect Ratio: {aspect_ratio}"
        )
        ...
        if "flux-schnell" in model_id:
            ...
            input_params.update(
                {"guidance": 3.5, "num_outputs": 1, "output_format": "webp", "output_quality": 100}
            )
        ...
        prediction = await loop.run_in_executor(
            None, lambda: replicate.run(model_id, input=input_params)
        )
        ...
        image_data = await loop.run_in_executor(None, lambda: requests.get(output_url).content)
        image_hash = hashlib.md5(image_data).hexdigest()
...
async def generate_image_gpt_image_1(prompt, aspect_ratio):
    try:
        logger.debug(
            f"Generating image with GPT Image 1. Prompt: {prompt}, Aspect Ratio: {aspect_ratio}"
        )
...
async def generate_image_gpt_image_1_edit(image_path, instruction, aspect_ratio):
    try:
        logger.debug(
            f"Editing image with GPT Image 1. Edit: {instruction}, Aspect Ratio: {aspect_ratio}"
        )
        size = _ASPECT_RATIO_TO_SIZE.get(aspect_ratio, "1024x1024")
