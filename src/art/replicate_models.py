...
class ReplicateModelSearch:
    ...
    async def search_models(self, query: str, limit: int = 20) -> List[Dict]:
        ...
        try:
            ...
            # If no results found, return popular models
            if not models:
                logger.info(f"No models found for query '{query}', returning popular models")
                return self.get_popular_models()
    ...
    def _is_image_model(self, model: Dict) -> bool:
        ...
        image_keywords = [
            'image', 'generate', 'text-to-image', 'diffusion', 'flux', 'sdxl', 
            'stable', 'dall', 'midjourney', 'art', 'photo', 'picture', 'visual'
        ]
        
        # Exclude non-image models
        exclude_keywords = [
            'text-to-speech', 'tts', 'audio', 'speech', 'voice', 'sound',
            'translation', 'language', 'chat', 'completion', 'llm'
        ]
    ...
    async def get_popular_models(self) -> List[Dict]:
        ...
        try:
            ...
            # Check cache first
            if (self.cached_popular_models and 
            ...
            for query in popular_queries:
                models = await self.search_models(query, limit=10)
    ...
    async def get_model_info(self, model_id: str) -> Optional[Dict]:
        ...
        try:
            ...
            model = await loop.run_in_executor(
                None, 
                lambda: replicate_client.models.get(model_id)
            )
