...
class DrawButtons(discord.ui.View):
    ...
    async def dalle_button(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        await interaction.response.edit_message(
            content="Select the aspect ratio for DALL-E 3:", view=self.aspect_ratio_view
        )
    ...
    async def gpt_image_1_button(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        await interaction.response.edit_message(
            content="Select the aspect ratio for GPT Image 1:", view=self.aspect_ratio_view
        )

    @discord.ui.button(label="Stable Diffusion 3", style=discord.ButtonStyle.secondary)
    async def stable_diffusion_button(
        self, interaction: discord.Interaction, button: discord.ui.Button
    ):
        if not self.aspect_ratio_view:
            self.aspect_ratio_view = AspectRatioView(self)
        await interaction.response.edit_message(
            content="Select the aspect ratio:", view=self.aspect_ratio_view
        )
        ...
        if not self.interaction_completed:
            try:
                await self.interaction.edit_original_response(
                    content="Image generation canceled due to timeout", view=None
                )
    ...
    async def generate_sd_image(self, interaction, aspect_ratio):
        await self.generate_image(
            interaction, "Stable Diffusion 3", image_generation.generate_image_sd, aspect_ratio
        )

    async def generate_replicate_image(
        self, interaction, aspect_ratio, model_id="black-forest-labs/flux-schnell"
    ):
        # Create a wrapper function that includes the model_id parameter
        async def replicate_with_model(prompt, aspect_ratio_param):
            return await image_generation.generate_image_replicate(
                prompt, aspect_ratio_param, model_id
            )
    ...
    async def generate_gpt_image_1_image(self, interaction, aspect_ratio):
        try:
            ...
            await interaction.edit_original_response(
                content=f"Generating image with GPT Image 1 (Aspect Ratio: {aspect_ratio})... This may take a minute or two.",
                view=None,
            )
            ...
            if isinstance(result, str):
                ...
                try:
                    await interaction.edit_original_response(
                        content=f"> **Error in GPT Image 1: {result}**", view=None
                    )
                except (discord.errors.NotFound, discord.errors.InteractionResponded):
                    logger.warning(
                        "Cannot edit interaction response - interaction expired or already responded"
                    )
                self.interaction_completed = True
                self.stop()
            else:
            ...
            try:
                await interaction.edit_original_response(
                    content="> **Error: An unexpected error occurred while generating the image with GPT Image 1.**",
                    view=None,
                )
            except (discord.errors.NotFound, discord.errors.InteractionResponded):
                logger.warning(
    ...
    async def generate_image(self, interaction, model_name, generate_function, aspect_ratio=None):
        try:
            await interaction.response.edit_message(
                content=f"Generating image with {model_name}...", view=None
            )
            ...
            if isinstance(result, str):
                ...
                try:
                    await interaction.edit_original_response(
                        content=f"> **Error in {model_name}: {result}**", view=None
                    )
                except (discord.errors.NotFound, discord.errors.InteractionResponded):
                    logger.warning(
                        "Cannot edit interaction response - interaction expired or already responded"
                ...
                await interaction.edit_original_response(
                    content=None, attachments=[file], embed=embed, view=view
                )
                self.interaction_completed = True
                self.stop()
            ...
            try:
                await interaction.edit_original_response(
                    content=f"> **Content Moderation Error: {str(e)}**", view=None
                )
            except (discord.errors.NotFound, discord.errors.InteractionResponded):
                logger.warning(
                    "Cannot edit interaction response - interaction expired or already responded"
            ...
            logger.error(
                f"Unexpected error in generate_{model_name.lower().replace(' ', '_')}_image: {str(e)}"
            )
            try:
                await interaction.edit_original_response(
                    content=f"> **Error: An unexpected error occurred while generating the image with {model_name}.**",
                    view=None,
                )
            except (discord.errors.NotFound, discord.errors.InteractionResponded):
                logger.warning(
...
class IterateImageModal(discord.ui.Modal, title="Iterate on Image"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        try:
            ...
            result = await generate_image_gpt_image_1_edit(
                self.image_path, self.instruction.value, self.aspect_ratio
            )
            ...
            await interaction.followup.send(
                content="✅ **Image iteration completed!**", file=file, embed=embed, view=view
            )
            ...
            try:
                await interaction.followup.send(
                    f"❌ An error occurred while editing the image: {str(e)}", ephemeral=True
                )
    ...
    async def iterate_button(self, interaction: discord.Interaction, button: discord.ui.Button):
        try:
            ...
            try:
                modal = IterateImageModal(self.prompt, self.image_path, self.aspect_ratio)
                await interaction.followup.send(
                    "The original interaction expired, but here's your iterate modal:",
                    view=None,
                    ephemeral=True,
                )
                # We can't send a modal through followup, so we need a different approach
                await interaction.followup.send(
                    "The original interaction has expired. Please use the `/draw` command again to generate a new image to iterate on.",
                    ephemeral=True,
                )
            except Exception as e:
