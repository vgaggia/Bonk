...
class ModelSelectionView(discord.ui.View):
    ...
    def update_buttons(self):
        ...
        if len(self.models) > self.models_per_page:
            if self.current_page > 0:
                prev_button = discord.ui.Button(
                    label="◀ Previous", style=discord.ButtonStyle.secondary, row=4
                )
        ...
        back_button = discord.ui.Button(
            label="🔍 New Search", style=discord.ButtonStyle.primary, row=4
        )
    ...
    async def cancel(self, interaction: discord.Interaction):
        try:
            await interaction.response.edit_message(
                content="Model selection cancelled.", embed=None, view=None
            )
        except (discord.errors.NotFound, discord.errors.InteractionResponded):
            logger.warning(
                "Cannot edit interaction response - interaction expired or already responded"
...
class ModelButton(discord.ui.Button):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        aspect_ratio_view = AspectRatioView(original_view, model="replicate", model_info=self.model)
...
class ReplicateModelSelectorView(discord.ui.View):
    ...
    async def flux_dev_model(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        # Create aspect ratio view for the selected model
        aspect_ratio_view = AspectRatioView(original_view, model="replicate", model_info=flux_model)
    ...
    async def cancel(self, interaction: discord.Interaction, button: discord.ui.Button):
        try:
            await interaction.response.edit_message(content="Model selection cancelled.", view=None)
        except (discord.errors.NotFound, discord.errors.InteractionResponded):
            logger.warning(
                "Cannot edit interaction response - interaction expired or already responded"
            )
